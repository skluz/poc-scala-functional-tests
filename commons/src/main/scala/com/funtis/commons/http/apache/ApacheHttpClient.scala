...
/**
  ...
  */
class ApacheHttpClient extends com.funtis.commons.http.HttpClient {
  ...
  override def execute(request: Request, configuration: Configuration): Response = {
    ...
    val responseHeaders = new Headers(clientResponse.getAllHeaders.map(h => com.funtis.commons.http.Header(h.getName, h.getValue)): _*)
